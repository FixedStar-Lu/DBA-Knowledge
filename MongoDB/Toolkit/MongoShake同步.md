...
### Mongoshake概述
MongoShake是采用GO语言编写的MongoDB数据同步订阅工具，其通过读取oplog操作日志来捕获数据，业务通过消费对应的日志数据来实现不同的场景，例如灾备或多活。项目地址：[mongoshake](mongoshake)
...
![one](https://github.com/alibaba/MongoShake/raw/develop/resources/dataflow.png)
...
[DownLoad](https://github.com/alibaba/MongoShake/releases)

**解压安装包**
```
[root@test-luhx ~]# tar -xvf mongo-shake-v2.6.4.tar.gz
[root@test-luhx ~]# mv mongo-shake-v2.6.4 /usr/local/mongo-shake
[root@test-luhx ~]# echo "export PATH=$PATH:/usr/local/mongo-shake" >> /etc/profile
[root@test-luhx ~]# source /etc/profile
```

**编辑配置文件**
```
[root@test-luhx media]# cat /usr/local/mongo-shake/collector.conf
...
log.dir = /service/mongodb/mongo-shake/logs
# log file name.
# log文件名。
...
mongo_urls = mongodb://root:Abcd123#@192.168.56.10:27017
...
tunnel.address = mongodb://root:Abcd123#@192.168.56.10:27018
...
filter.namespace.white =test;db2.users;
...
checkpoint.storage.url =mongodb://root:Abcd123#@192.168.56.10:27018
...
**启动同步进程**
```
[root@test-luhx ~]# sh start.sh /usr/local/mongo-shake/collector.conf
```
...
查看全量同步进度
```
[root@test-luhx ~]# curl -s  http://127.0.0.1:9101/progress | python -m json.tool
```

配置限速
```
[root@test-luhx ~]# curl -s 127.0.0.1:9101/sentinel | python -m json.tool
```
...
查看增量复制
```
[root@test-luhx ~]# curl -s http://127.0.0.1:9100/repl | python -m json.tool
```
也可以通过mongoshake-stat --port=9100的方式查看复制信息
...
对于加入同步前就已经存在的集合，因为其本身就存在数据，无法单独全量同步到目标端，因此得先处理这部分存量数据。这里我们可以考虑mongodump+checkpoint+oplog的幂等性来实现添加

1. 记录当前checkpoint点
```
target:PRIMARY> use mongoshake
target:PRIMARY> db.ckpt_default.find()
{ "_id" : ObjectId("609e31ffbc509541e0814aec"), "name" : "source", "ckpt" : Timestamp(1620982316, 1), "fetch_method" : "oplog", "oplog_disk_queue" : "", "oplog_disk_queue_apply_finish_ts" : Timestamp(0, 0), "version" : 2 }
target:PRIMARY> var timestamp = Timestamp(1620982316, 1)
target:PRIMARY> print( new Date( timestamp.t*1000+timestamp.i))
Fri May 14 2021 16:51:56 GMT+0800 (CST)
```

2. mongodump导出数据
```
[root@test-luhx ~]# mongodump --host 192.168.56.10 --port 27017 --username root --password "Abcd123#"  --authenticationDatabase admin --db db1 --out /service/mongodump-2021-05-14
```

3. mongorestore导入数据
```
[root@test-luhx ~]# mongorestore --uri=mongodb://root:Abcd123#@192.168.56.10:27018 /service/mongodump-2021-05-14/
```

4. 修改参数文件中的checkpoint.start_position为上面的checkpoint time，并添加新增表到filter.namespace.white中

5. 清空ckpt_default集合
```
use mongoshake
db.ckpt_default.deleteMany({})
```

6. 重启mongoshake进程
```
[root@test-luhx ~]# sh stop.sh /service/mongodb/mongo-shake/logs/mongoshake.pid
[root@test-luhx ~]# sh start.sh /usr/local/mongo-shake/collector.conf
```

**白名单+新创建的集合**

对于新创建的表，我们可以在创建之前就将集合加入到配置文件的filter.namespace.white中，再在源端创建集合即可同步


### Checkpoint的理解

Checkpoint是记录增量同步位点的信息，mongoshake启动时就是根据这个位点来同步的，例如我们已经同步了10个小时，如果这时候进程发生重启，没有位点信息就只能从头开始拉取。在mongoshake中是通过64位时间戳来标识，如果发生重启就会去源库oplog查找该位点，并以此为同步起始点。

Checkpoint存储是根据参数checkpoint.storage.url来设置的，会定期进行存储，如果有数据同步，checkpoint频率会很快(秒级别)，如果没有数据同步会分钟级别更新checkpoint